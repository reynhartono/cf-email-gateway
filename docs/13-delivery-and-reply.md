...
Authserv allowlist alone is not enough when several Cloudflare-looking lines exist (issue #22): a client-supplied `Authentication-Results: mx.cloudflare.net; …pass…` preserved next to Email Routing's own evaluation must not authorize. Every Cloudflare-looking line that mentions the envelope identity must itself carry an aligned pass — any such line without one (fail / none / softfail / temperror / permerror) vetoes, order-independently. Lines that do not mention the identity are ignored. This deliberately prefers strictness over availability: an injected CF-looking fail line fails closed (mail stays on default forward) rather than risk spoofed auth — the alternative of trusting only the "last" CF line depends on header-insertion order and is not used. Residual: a lone preserved client `mx.cloudflare.net` pass with no Email Routing evaluation for that identity still authorizes this gate (unchanged); the envelope-From allowlist / identity bind / `can_send_as` / token gates still apply, and whether Routing preserves such client AR vs always emitting its own evaluation is pending live verification.
