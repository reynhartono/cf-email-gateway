...
| Match | `local_part_prefix` and/or `local_part_plus` + exact `address` as needed |
...
| Person + plus tag | `bob+github@example.com` | `local_part_plus` `value: bob` |
...
`address` → `local_part_*` → `catch_all` → `default_inbox`  
